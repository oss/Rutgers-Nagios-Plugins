...
def ldap_connect(ldapuri, logger=None, binddn="", timeout=None):
  """
  Perform LDAP connection and synchronous simple bind operation
    ...
    timeout - Seconds until connection times out
  ...
  if timeout: conn.timeout=eval(timeout)
  conn.start_tls_s()
...
def is_insynch(provldapobj, consldapobj, basedn, threshold=None, logger=None):
  ...
  if (provcontextCSN and conscontextCSN):
    if (provcontextCSN == conscontextCSN):
      ...
      if threshold:
        ...
        if (abs(delta) <= maxdelta):
          if logger:
            ...
            print "OK:  Consumer in SYNCH within threshold, Delta is %s" % delta;
          return True
        else:
      ...
      print "CRITICAL:  Consumer NOT in SYNCH, Delta is %s" % delta;
  else:
    if logger: logger.error("  Check failed: at least one contextCSN value is missing")
    print "CRITICAL:  Check failed: at least one contextCSN value is missing";
  return False
...
def main():
  ...
  ldapprov = ldap_connect(args.pop(0), logger, options.binddn, options.timeout)
  if ldapprov:
    for consumer in args:
      ...
      if ldapcons:
        insync = is_insynch(ldapprov, ldapcons, options.basedn, options.threshold, logger)
        ldapcons.unbind_s()
    ldapprov.unbind_s()
